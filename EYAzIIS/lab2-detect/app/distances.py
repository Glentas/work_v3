...
Все метрики возвращают число, которое МИНИМИЗИРУется: категория с наименьшим
значением принимается за язык документа (схема методички).
"""
...
def out_of_place(doc: NgramProfile, lang: NgramProfile) -> int:
    """Мера несовпадения позиций N-грамм (Cavnar–Trenkle, рис. 1 методички).
