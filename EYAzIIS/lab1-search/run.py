"""Запуск сервера информационно-поисковой системы.

Сервер поднимается на главном ПК и слушает все сетевые интерфейсы,
поэтому остальные машины локальной сети обращаются к нему по адресу
...
def print_banner(host: str, port: int) -> None:
    ...
    print(f"  Коллекция:     {config.COLLECTION_PATH}")
    print(f"  База данных:   {config.DB_PATH}")
    print("  Для остановки нажмите Ctrl+C.\n")


def main() -> None:
    parser = argparse.ArgumentParser(description="Сервер информационно-поисковой системы")
