...
Все параметры переопределяются переменными окружения — это позволяет
запускать сервер на одной машине (главный ПК), а клиентам в локальной
сети обращаться к нему по адресу http://<host>:<port>.
"""
...
# --- Поисковая стратегия ----------------------------------------------------
# Стратегия с отказами (методичка, раздел «Модель поиска документов»):
