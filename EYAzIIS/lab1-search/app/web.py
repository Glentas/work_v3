"""Веб-интерфейс и HTTP API информационно-поисковой системы.

Страницы:

===========  ==========================================================
``/``        форма поиска
``/search``  результаты поиска с разметкой релевантности
``/metrics`` таблицы и графики оценок качества
``/help``    справка по работе с системой
===========  ==========================================================

JSON API используется интерфейсом для отметок релевантности и построения
графиков, а также доступно внешним клиентам в локальной сети.

Сервер слушает все сетевые интерфейсы (``APP_HOST=0.0.0.0``), поэтому
запускать его достаточно на одном компьютере — остальные участники
локальной сети работают с ним через браузер.
"""
