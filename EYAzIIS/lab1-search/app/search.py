"""Поисковый механизм: логический отбор документов и ранжирование выдачи.

Стратегия поиска — логическая (вариант 7). Запрос пользователя сводится к
булеву выражению над инвертированным индексом:

* ``all_words``  — конъюнкция: документ должен содержать **все** термины;
* иначе          — дизъюнкция: достаточно **любого** термина;
* ``-термин``    — отрицание: документ с таким термином исключается;
* даты           — дополнительное ограничение по дате добавления документа.

Отобранные документы упорядочиваются по возрастанию релевантности —
это ключевая задача линейной модели поиска из методички. Порядок определяется
весами, вычисленными при индексировании по формуле 1.6 (A_i^j = Q_i^j * B_i):

1. число терминов запроса, найденных в документе;
2. сумма весов A_i^j по совпавшим терминам;
3. идентификатор документа — гарантирует детерминированный порядок при
   равенстве первых двух ключей. Без него выдача зависела бы от порядка
   обхода множества, и оценки качества оказались бы невоспроизводимыми.

При поиске по «всем словам вместе» первый критерий одинаков для всех
документов, поэтому фактически работает сумма весов.

Стратегия с отказами. Методичка указывает, что простейший способ поиска по
естественно-языковому запросу — сведение его к логическому, «как правило,
это стратегия с отказами». Если конъюнкция не дала ни одного документа
(в том числе потому, что части слов вообще нет в словаре коллекции),
система автоматически ослабляет условие до дизъюнкции и сообщает об этом
пользователю вместо пустой выдачи.
"""
