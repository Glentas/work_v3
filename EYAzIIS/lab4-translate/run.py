"""Запуск сервера системы машинного перевода.
...
def print_banner(host: str, port: int) -> None:
    ...
    print(f"  База словаря:  {config.DB_PATH}")
    print(f"  Языковая пара: {config.SOURCE_LANG_NAME} -> {config.TARGET_LANG_NAME} (вариант 7)")
    print("  Для остановки нажмите Ctrl+C.\n")


def main() -> None:
    parser = argparse.ArgumentParser(description="Сервер машинного перевода текстов")
